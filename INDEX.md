...
| `app/services/free_turn.py` (or inline in main.py) | Free-turn continuation scheduler: `free_turn_followups` scoped-SQLite table for scheduled wakes; `whatsapp_pending_outbounds` table for durable WhatsApp delivery; FastAPI lifespan scheduler claims due rows and fires fresh retrieve→turn. |
