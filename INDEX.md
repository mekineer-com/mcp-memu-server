...
| `/conversation/{id}/turn` | POST | Soul turn loop: run LLM with turn contract, persist intentions + cache; system identity uses ST `soul_card` when provided, otherwise self-model-derived card (`narrative_self`); retrieve route context prepends an identity/time anchor block (`identity_context`) before soul-context queries; turn prompt history prefers 2*`chat_x` anchor span when anchors resolve, else falls back to full normalized turn history (token-budgeted at render time); queues forced memorize when unmemorized chat tail exceeds `memorize.turn_history_token_budget` |
