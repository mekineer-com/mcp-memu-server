...
| `/conversation/{id}/turn` | POST | Soul turn loop: run LLM with turn contract, persist intentions + cache; system identity uses ST `soul_card` when provided, otherwise self-model-derived card (`narrative_self`); turn prompt history prefers 2*`chat_x` anchor span when anchors resolve, else falls back to sleep-gap-sliced history; queues forced memorize when unmemorized chat tail exceeds `memorize.turn_history_token_budget` |
