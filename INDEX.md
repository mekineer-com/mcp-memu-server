...
| `app/services/conversation_sources.py` | Source adapters: WhatsApp from Hermes Channels (`sessions.json` + `state.db`), ST from `st_chats/*/latest_history.json`. Handles cursor slicing, floor backfill, and role normalization at the memU boundary. |
