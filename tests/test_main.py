...
@pytest.mark.asyncio
async def test_conversation_turn_falls_back_to_user_id_when_user_name_missing(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """When the caller (e.g., SillyTavern plugin) omits user_name, the speaker
    falls back to user_id so cross-chat rendering never shows a bare "[user]:"
    label.
    """
    db_path = tmp_path / "Echo.db"
    con = main._sqlite_connect(db_path)
    ...
    class _FakeSvc:
        async def chat(self, *_args, **_kwargs) -> str:
            return '{"cache":null,"annulments":[],"inner_thought":"ok","response":"hi back"}'
    ...
    payload = {
        "user": {"user_id": "Marcos", "soul_id": "Echo", "conversation_id": "cid-st"},
        "message": "Getting the hang of having many chats at once?",
        # user_name intentionally omitted (mirrors the SillyTavern plugin payload).
        "history": [],
        "run_apimw": False,
        "apply_turn_maintenance": False,
        "prompt_override_payload": {
    ...
    out = await main.conversation_turn("cid-st", payload)
    assert out["ok"] is True
    ...
    assert len(rows) == 2
    assert str(rows[0]["role"]) == "user"
    assert str(rows[0]["speaker"]) == "Marcos"
    assert str(rows[1]["role"]) == "assistant"
    assert str(rows[1]["speaker"]) == "Echo"
