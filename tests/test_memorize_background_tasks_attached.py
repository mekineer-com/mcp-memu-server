...
History: the endpoint adds `_run_memorize_batches` via `background_tasks.add_task(...)`
...
This test is the regression guard: it stubs `_run_memorize_batches` with a
...
def test_memorize_background_task_runs(client: TestClient, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    async def fake_run(**kwargs) -> None:
        recorded.append({"ran": True, "batch_count": len(kwargs.get("memorize_batches") or [])})

    # Stub the heavy work so we can verify the task fires without LLM calls.
    monkeypatch.setattr(main_module, "_run_memorize_batches", fake_run)
    ...
    assert recorded[0]["batch_count"] >= 1
